...
import { peek, initSocket, onRecv } from "./socket";
...
let outDiv = document.querySelector(".crop-recomend-grap");

const CITY = "Thane";

// TODO remove later (like seriously huh)
document.querySelector(".avg-temp").innerHTML = CITY;
...
document.querySelector(".avg-temp").innerHTML = 20;
...
onRecv(() => {
  ...
  fetch("/recommend", {
    ...
    body: JSON.stringify([
      ...
      25, districts[CITY].humidity,
      ph[index], districts[CITY].rainfall,
      0 // Rabi
    ])
  }).then(res => res.json()).then(data => {
    let cropData = crops.map((crop, index) => ({ crop: crop, value: data[index] }));
    cropData.sort((a, b) => a.value - b.value);

    let sortedCrops = cropData.map(pair => pair.crop);
    outDiv.innerHTML =
      `<div class="crop1">${sortedCrops[0]}</div><br/>` +
      `<div class="crop2">${sortedCrops[1]}</div><br/>` +
